...
1. **沙箱静态分析**：Shell-aware 解析器，五级风险评估（ReadOnly → LowRisk → Moderate → HighRisk → Destructive），内置黑名单/白名单
2. **模型审批**：LLM 独立判断命令安全性，支持放行/转人工/阻止三种决策
3. **人工审批**：弹出审批对话框，显示风险级别、命令内容、模型提示原因，支持快捷键操作
