...
1. **命令风险评估（风险名单用户可自定义）**：Shell-aware 解析器，五级评估（ReadOnly → LowRisk → Moderate → HighRisk → Destructive）
...
- **原生终端体验**：流畅渲染、符合 Windows 习惯的 `Ctrl+C`/右键粘贴交互，拒绝为 AI 功能牺牲操作效率。同时支持点击跳转光标位置，操作简单顺手。
- **完整的 SFTP 文件管理**：拖拽上传/下载、文件夹操作、在线解压与编辑，不依赖第三方软件。
- **严苛的安全体系**：三层审批（风险评估、模型判断、人工确认）+ 密钥链隔离 + 内存敏感数据清零，危险命令无处遁形。
- **插件生态**：插件系统扩展无限可能，WebView 面板、命令注入与事件监听让能力按需生长。
...
很多 SSH客户端在手机上就是个"能用就行”的水平，切后台就断连，Agent 任务跑一半没了。Marcel SSH在 Android 端下了真功夫:

Android 端做了前台服务保活，切后台 SSH 不断连，Agent 任务照跑。
