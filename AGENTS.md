...
- **Windows 发布**：默认只构建/发布 NSIS 安装包，不发布 MSI；如需 MSI，必须在发布前显式说明并临时调整 Tauri bundle target。
...
- 用户偏好中文、直接、少废话；不要用大段模板化解释。用户不喜欢长篇假设列表。
- 用户不喜欢过度保守；基础工具应按“真正好用”设计，不能过度遵循最小化设计。
- 用户会使用简写口令：`c p` / `cm ph` 表示提交并推送；`l c p` 表示先检查变更、按功能拆分为独立 commit、最后推送。提交前必须检查 `git status`、`git diff`、`git log --oneline -10`。
- Git 提交信息用中文描述，可保留 `chore:` / `feat:` / `fix:` 前缀。
- 如果不确定用户意图，先问一个短问题，不要展开长篇分支分析，更不要瞎猜然后随便执行。