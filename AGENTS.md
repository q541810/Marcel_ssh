...
5. 本文件是 **绝对权威** 是 **不可违反** 的，必须遵循本文件（除非你把本文件改成符合你需求的样子）

6. 你不能执行任何预计耗时一秒以上的shell命令，但可以要求用户帮你执行（或者用户要求你这样做，使用智能体代理时一定要提醒他这个）

7. 修改版本号时必须同时更新 `src-tauri/tauri.conf.json` 和 `package.json`
