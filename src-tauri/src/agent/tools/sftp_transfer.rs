...
// Re-use the base64 codec from file_ops (private): we inline tiny copies here
// so this module stays self-contained and file_ops doesn't need to expose
// internal helpers.
...
impl AgentTool for DownloadFileTool {
    ...
    async fn execute(
        ...
        let bytes = match b64_decode(b64.trim()) {
            Ok(b) => b,
            Err(e) => return Ok(ToolOutput::fail(
                format!("download {}", remote_path),
