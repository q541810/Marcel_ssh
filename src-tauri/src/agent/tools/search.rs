...
impl SearchFilesTool { pub fn new() -> Self { Self } }
impl Default for SearchFilesTool { fn default() -> Self { Self::new() } }
    ...
    async fn execute(
        ...
        match ctx.exec(&cmd).await {
            Ok(output) => {
                ...
                Ok(ToolOutput::ok(
                    format!("search '{}' ({} match{})", pattern, count, if count == 1 { "" } else { "es" }),
                    body,
                )
                .with_metadata(json!({
                    "matches": count,
