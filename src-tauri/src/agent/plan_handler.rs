...
pub(crate) async fn handle_update_plan_item(
    ...
    if is_terminal_transition && !plan.reflection_reminded && is_plan_complete(plan) {
        ...
        let reminder = format!(
            "你尝试把 plan 的最后一步（{item_id}「{title}」）标记为终态的操作已被系统拦截：本次状态变更已回滚，该步骤仍未收尾，任务不会就此结束。\n\
             \n\
             请对照会话里已经出现的真实结果逐项核对：\n\
             - 任务声明的每个目标，都要有对应的可验证证据（命令输出、文件内容、请求结果、搜索结果等），且必须是**用工具实际执行**得到的，不是在报告里描述的\n\
             - 证据须来自用户可感知的结果（实际访问、实际输出、实际内容），而不是中间状态（进程在跑、端口在监听、任务在运行）\n\
             - 任何目标未达成或无法提供证据，都不得标 completed：标 failed 并写清原因，或用 edit_plan 增补步骤\n\
             \n\
             接下来按这个顺序做，不要颠倒（在 plan 收尾之前写报告会被系统打回）：\n\
             1. 先用工具把证据补齐——跑命令 / 读文件 / 发请求，把真实结果留在会话里\n\
             2. 证据齐了，调用 update_plan_item 再次标记终态（这次不会再拦截）\n\
             3. plan 全部收尾之后，才写最终报告"
        );
