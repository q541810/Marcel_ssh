...
impl SshManager {
    ...
    /// Get a cloneable reference to the internal connections map.
    /// Used by agent tasks that need SSH access from spawned tasks.
    pub fn clone_inner(&self) -> SshManagerClone {
        SshManagerClone {
...
/// A cloneable handle to the SSH connection pool that can be sent to spawned tasks.
#[derive(Clone)]
pub struct SshManagerClone {
    connections: Arc<RwLock<HashMap<String, Arc<SshConnection>>>>,
}

impl SshManagerClone {
    /// Execute a command via a dedicated exec channel.
    pub async fn exec_command(
        &self,
        session_id: &str,
        command: &str,
