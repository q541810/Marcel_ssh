...
- **一键安装**：市场详情页点击「一键安装」，应用自动下载插件源码 zip 并解压到插件目录（仅支持 GitHub 仓库；已安装同 id 插件时拒绝，需先卸载）。点击后弹出全屏覆盖层：显示下载/解压进度条，可随时**取消**（取消后清理临时文件，无残留）；覆盖层在任务结束前无法关闭，避免错过「重启后生效」提示。安装完成需**重启应用**生效。
...
- 安装/卸载后到重启前，市场页的「已安装」状态由前端会话级记录保持一致（不刷新插件列表），重启后以真实插件目录为准。
