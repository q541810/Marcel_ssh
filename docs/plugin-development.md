...
- **一键安装**：市场详情页点击「一键安装」，应用自动下载插件源码 zip 并解压到插件目录（仅支持 GitHub 仓库；已安装同 id 插件时拒绝，需先卸载）。安装完成需**重启应用**生效。
- **卸载**：市场详情页或 **设置 → 插件** 均可卸载。卸载会删除插件目录及其全部数据（如记忆文件），并清理权限授权与禁用设置残留，重启应用后完全移除。
