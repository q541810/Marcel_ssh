...
重启 Marcel SSH，左侧导航栏会出现 "Hello" 按钮，点击即可看到插件界面。

> **注意：插件不会在「设置 → 插件 → 刷新」后加载**。刷新只更新插件列表与
> 注册状态；插件运行时（Agent 工具、内容脚本、HTTP API、独立窗口）在应用
> 启动时加载，新放入目录或新安装的插件必须**重启应用**才会生效。
>
> 市场页「一键安装」/ 卸载完成后，应用**不会**自动刷新插件列表（避免刷新
> 波及其他插件的运行时），需重启应用后插件才会加载（或完全移除）。安装期间
> 会弹出全屏进度覆盖层（下载/解压进度条，可取消），安装/卸载进行中无法关闭
> 该覆盖层，完成后会展示「重启应用后生效/移除」提示。
...
修改插件文件后，**重启应用**生效。插件运行时在应用启动时加载，
「设置 → 插件 → 刷新」只更新插件列表与注册状态，不会重新加载插件。
...
> `<app-config-dir>` 因平台而异（详见 [快速开始](#1-创建插件目录)）。复制后**重启应用**即可。
...
1. **安装插件**：复制示例目录到 `<app-config-dir>/plugins/long-term-memory/`，重启应用
...
- **`plugin.json` 必须位于插件仓库根目录**（市场索引从它提取 id/version/name/description/minAppVersion/capabilities/preservePaths）
- 必须声明 `preservePaths`（即使无个人数据也需 `"preservePaths": []`，否则审核打回）；个人数据路径如 `config.json`、`memories/`、`data/` 需在此列出（`config.json` 未声明也会隐式保留，但仍建议显式声明）
...
- `repoUrl` 指向插件仓库，应用内「一键安装」自动下载源码 zip（镜像优先）并解压到插件目录
- `icon`：`{ "kind": "emoji" | "img", "value": "..." }`
- 市场镜像可配置（市场页右上角「默认/自定义」）：填写 GitHub 加速镜像前缀（如 `https://ghfast.top`）后，市场列表、详情、图片与插件下载统一走该镜像；留空使用内置默认（jsDelivr 拉取列表 + 内置镜像列表下载 + GitHub 直连兜底）

### 应用内安装 / 卸载 / 更新

- **一键安装**：市场详情页点击「一键安装」，应用自动下载插件源码 zip 并解压到插件目录（仅支持 GitHub 仓库；已安装同 id 插件时拒绝，需先卸载）。点击后弹出全屏覆盖层：显示下载/解压进度条，可随时**取消**（取消后清理临时文件，无残留）；覆盖层在任务结束前无法关闭，避免错过「重启后生效」提示。安装完成需**重启应用**生效，安装时写入 `.marcel-shipped.json` 清单用于后续更新区分代码与数据。
- **更新**：**设置 → 插件** 对已安装且市场有新版的插件显示“更新”按钮（`v本地 → v市场`），也可在市场详情页跳回设置更新。更新同样走下载/解压覆盖层，**保留个人数据**：①旧清单上没有的用户新建文件自动保留 ② `preservePaths` 命中的文件即使在清单上也保留（`config.json` 隐式保留）。旧版多余代码文件不残留；首次更新无清单的旧插件按“新包没有的本地文件全保留”兜底。更新同样需重启生效；失败/取消回滚不丢数据。可手动“检查更新”。
- **卸载**：市场详情页或 **设置 → 插件** 均可卸载。卸载同样以全屏覆盖层进行（不可关闭、不可取消），完成后提示「重启应用后完全移除」。卸载会删除插件目录及其全部数据（如记忆文件），并清理权限授权与禁用设置残留。
- 安装校验：解压路径做 zip-slip 防护；`plugin.json` 必须位于压缩包根目录（GitHub archive 的 `repo-main/` 根目录层会自动剥离）；插件 id 仅允许字母、数字、连字符、下划线。
- 安装/更新/卸载后到重启前，市场页的「已安装」状态由前端会话级记录保持一致（不刷新插件列表），重启后以真实插件目录为准。
