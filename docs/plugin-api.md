...
## minAppVersion（版本兼容声明）

插件声明自己依赖的最低应用版本，防止装了新 API 功能的插件跑在旧版应用上静默缺功能。可选；未声明时插件无条件兼容。

```jsonc
{
  "id": "my-plugin",
  "minAppVersion": "1.7.0"   // 依赖 window.create 等 1.7.0 才有的能力
}
```

**行为规则**

- **自动禁用**：运行的应用版本低于 `minAppVersion` 时，插件**不会加载**——Agent 工具不注册、内容脚本不注入、HTTP API / 事件 IPC / 独立窗口全部拒绝。设置页仍显示该插件，头部标记 `需要应用 vX`，启用开关置灰，并提示升级应用。
- **不写用户设置**：不兼容插件不会被写入 `disabled_plugins`。应用升级到兼容版本后，重启应用自动恢复启用，无需用户干预。
...
> **平台限制**：插件系统仅支持桌面端。Android 移动端整体不兼容插件——没有插件安装/管理入口，不挂载视图、不注入内容脚本、不渲染配置 UI，插件相关 IPC（如 `plugin_webview_*`）在移动端返回错误。
...
| `agent://stream/*`                                                    | Agent 任务流                                                                                                                                                                  | `StreamEvent`（textDelta/toolCall/done/cancelled/error；`done` = 模型自然结束，`cancelled` = 被取消停止）                            |
