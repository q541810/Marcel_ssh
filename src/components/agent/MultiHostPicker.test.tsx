...
afterEach(() => {
  act(() => root?.unmount());
  container?.remove();
  // 弹层用 createPortal 挂到 body，卸载后清掉残留容器
  document.body.innerHTML = '';
});

/**
 * 隐私模式：目标机器清单里的 `user@host:port` 也要脱敏。
 *
 * 连接列表、快速连接、HostKey 提示都走 lib/privacy 的 formatConnLabel，多机选择器
 * 却是裸插值——开了隐私模式，主机名和端口照样明晃晃地摊在 Agent 面板顶栏的弹层里。
 * 这里把「当前机」和「可跨机目标」两种行都钉住。
 */
describe('多机目标清单的隐私模式', () => {
  it('关闭时显示真实地址', async () => {
    await openPicker();
