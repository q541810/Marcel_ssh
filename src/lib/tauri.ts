...
export interface CrashInfo {
  crashType: 'config_corrupted' | 'startup_failure' | 'runtime_panic' | 'database_error' | 'unknown';
  errorMessage: string;
...
export async function crashCheckPrevious(): Promise<CrashInfo | null> {
  return invoke<CrashInfo | null>('crash_check_previous');
}

export async function crashGetReport(): Promise<CrashReport | null> {
  return invoke<CrashReport | null>('crash_get_report');
}

export async function crashExportReport(exportPath: string): Promise<string> {
  return invoke<string>('crash_export_report', { exportPath });
}

export async function crashRepairConfig(fileName: string): Promise<void> {
  return invoke('crash_repair_config', { fileName });
}

export async function crashListBackups(fileName: string): Promise<ConfigBackup[]> {
  return invoke<ConfigBackup[]>('crash_list_backups', { fileName });
}

export async function crashRestoreBackup(backupPath: string): Promise<void> {
  return invoke('crash_restore_backup', { backupPath });
}

export async function crashDismiss(): Promise<void> {
  return invoke('crash_dismiss');
}

export async function crashMarkResolved(): Promise<void> {
  return invoke('crash_mark_resolved');
}
