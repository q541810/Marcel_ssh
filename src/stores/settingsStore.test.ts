...
describe("settingsStore", () => {
  ...
  it("default agent mode settings are correct", () => {
    ...
    expect(am.modelApprovalContextLevel).toBe("normal");
    expect(am.confirmEditFile).toBe(true);
  });

  it("fills the approval context default for old nested settings", async () => {
    const initial = useSettingsStore.getInitialState().settings;
    const { modelApprovalContextLevel: _omitted, ...oldAgentModeSettings } =
      initial.agentModeSettings;
    vi.mocked(tauri.getSettings).mockResolvedValue({
      settings: {
        ...initial,
        agentModeSettings:
          oldAgentModeSettings as typeof initial.agentModeSettings,
      },
      hasApiKey: false,
      hasWebSearchApiKey: false,
    });
