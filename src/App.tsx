...
export default function App() {
  ...
  const windowResizeTimeoutRef = useRef<ReturnType<typeof setTimeout> | null>(
    null,
  );
  ...
  const [resizingSide, setResizingSide] = useState<"sidebar" | "agent" | null>(
    null,
  );
  const [isWindowResizing, setIsWindowResizing] = useState(false);
  const agentPanelUnmountTimeoutRef = useRef<ReturnType<
    typeof setTimeout
  > | null>(null);
  ...
  const sidebarOpen =
    workspaceLayout?.sidebarOpen ?? DEFAULT_WORKSPACE_LAYOUT.sidebarOpen;
  const agentPanelOpen =
    workspaceLayout?.agentOpen ?? DEFAULT_WORKSPACE_LAYOUT.agentOpen;
  const disabledPlugins = useSettingsStore((s) => s.settings.disabledPlugins);
  const disableAllInjections = useSettingsStore(
    (s) => s.settings.disableAllInjections,
  );
  const authorizedCapabilities = useSettingsStore(
    (s) => s.settings.authorizedCapabilities,
  );
  const centerProviders = useMemo(
    () => byMount(providers, "center"),
    [providers],
  );
  const agentProviders = useMemo(
    () => byMount(providers, "agent"),
    [providers],
  );
  ...
  const effectiveSidebarOpen =
    sidebarOpen && !isExclusive && sidebarProvider !== null;
  const effectiveAgentPanelOpen =
    agentPanelOpen && !isExclusive && agentProvider !== null;
  const [agentPanelMounted, setAgentPanelMounted] = useState(
      ...
      if (resizingSide === "agent" && agentResizeStartRef.current) {
        ...
        const width = Math.min(
          ...
          Math.max(
            ...
            agentResizeStartRef.current.width + delta,
          ),
        );
  ...
  useEffect(() => {
    ...
    checkUpdate()
      .then((res) => {
        if (res.hasUpdate)
          setUpdateToast({ version: res.latestVersion, url: res.releaseUrl });
      })
      .catch(() => {});
          ...
          >
            <div style={{ width: `${sidebarWidth}px`, height: "100%" }}>
              {sidebarProvider && sidebarProvider.pluginId !== "builtin" ? (
                <PluginWebviewSlot
                  key={`${sidebarProvider.id}-${pluginRefreshKey}`}
            ...
            {isExclusive ? (
              <div className="flex-1 flex flex-col min-w-0 overflow-hidden bg-zinc-900 animate-settings-workspace-enter">
                {centerProvider && centerProvider.pluginId !== "builtin" ? (
                  <PluginWebviewSlot
                    key={`${centerProvider.id}-${pluginRefreshKey}`}
          ...
          <div
            ...
            style={{
              ...
              transition:
                isResizing || isWindowResizing
                  ? "none"
                  : "width 300ms var(--spring-bounce, cubic-bezier(0.34, 1.56, 0.64, 1))",
            }}
          >
              ...
              <>
                ...
                >
                  {agentProvider && agentProvider.pluginId !== "builtin" ? (
                    <PluginWebviewSlot
                      key={`${agentProvider.id}-${pluginRefreshKey}`}
