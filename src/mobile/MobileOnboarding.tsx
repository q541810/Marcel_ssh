...
/**
 * Mobile onboarding: welcome → model → touch guide。
 * 与桌面共享 `hasCompletedOnboarding` flag，
...
export default function MobileOnboarding({
  ...
  // Android back：steps 内上一步；第 0 步不接管 back，交给系统（退出应用）。
  const handleBack = useCallback(() => {
    setStep((s) => Math.max(0, s - 1));
  }, []);

  const isLast = step === STEP_COUNT - 1;
  // step 切换重挂载内容区，重播入场动画
  ...
  return (
    <SettingsActionsProvider value={actionsValue}>
      <MobileFullscreenPage
        ...
        // 第 0 步不接管 back，交给系统（退出应用）。
        onBack={step > 0 ? handleBack : undefined}
      >
        {/* Header：内容步骤（非第 0 步）显示返回 + 进度 + 跳过 */}
        {step > 0 && (
          <header
