...
export default function PluginWebviewSlot({ provider }: Props) {
  ...
  useEffect(() => {
    ...
    pluginWebviewCreate(label, provider.pluginId, entry, rect.x, rect.y, rect.width - 5, rect.height)
      .then(() => {
      ...
      .catch((err) => {
        console.error('[plugin-slot] create failed:', err);
        setError(String(err));
      });
    ...
    return () => {
      ...
      applyBounds(0, 0, 0, 0);
      if (createdRef.current) pluginWebviewClose(label).catch(console.error);
    ...
    return (
      <div ref={ref} className="w-full h-full flex items-center justify-center bg-zinc-900">
        <div className="text-center space-y-3 px-4">
          <div className="text-red-400 text-sm font-medium">插件加载失败</div>
